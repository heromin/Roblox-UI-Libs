--[[ 
    HYVERION - FATALITYUI
    - Compatible principalmente con Velocity, probablemente funcione en fluxus y algunos otros
    - Soporte para Sliders, Dropdowns y Toggles.
...
-- Intentar cargar configuración antes de construir UI
Load()

-- =========================================================
-- COMPATIBILIDAD UNIVERSAL (EXECUTOR FIX)
-- =========================================================
local function safe_mouse_move(x, y)
...
local function has_drawing_lib()
    return (Drawing and Drawing.new) ~= nil
end

local OriginalWalkSpeed = 16
local function SetOriginalSpeed(char)
    local hum = char:WaitForChild("Humanoid", 5)
    if hum then OriginalWalkSpeed = hum.WalkSpeed end
end

if LocalPlayer then
    table.insert(GlobalConnections, LocalPlayer.CharacterAdded:Connect(SetOriginalSpeed))
    if LocalPlayer.Character then task.spawn(SetOriginalSpeed, LocalPlayer.Character) end
end

local Notify -- Declarado aquí para que sea accesible en todo el script
...
-- =========================================================
-- SISTEMA DE HITMARKERS / HITLOGS
-- =========================================================
local ToggleObjects = {}

local function UpdateHitlogs()
    local center = Camera.ViewportSize / 2
    for i, drawing in ipairs(Drawings.HitNotifications) do
        drawing.Position = Vector2.new(center.X, (center.Y + 150) + (i * 18))
    end
end

local function AddHitlog(hitpart, username)
    if not Settings.ESP.HitLogs.Enabled or not has_drawing_lib() then return end
    
    task.spawn(function()
        local hitlog = Drawing.new("Text")
        hitlog.Size = 13
        hitlog.Font = 2
        hitlog.Text = string.format("[fatality.win] damage inflicted to %s on %s", username, hitpart)
        hitlog.Visible = true
        hitlog.ZIndex = 3
        hitlog.Center = true
        hitlog.Color = Settings.ESP.HitLogs.Color
        hitlog.Outline = true

        local sound = HitSound:Clone() -- Clonar el sonido para cada hit
        sound.Parent = game:GetService("SoundService")
        sound:Play()
        game:GetService("Debris"):AddItem(sound, 2) -- Limpiar el sonido después de 2 segundos

        table.insert(Drawings.HitNotifications, hitlog)
        UpdateHitlogs()
        
        task.wait(3)

        local index = table.find(Drawings.HitNotifications, hitlog)
        if index then
            table.remove(Drawings.HitNotifications, index)
            UpdateHitlogs()
            hitlog:Remove()
        end
    end)
...
-- Hook para juegos específicos (ProjectileInflict)
local oldNamecall
oldNamecall = hookmetamethod(game, "__namecall", function(obj, ...)
    local method = getnamecallmethod()

    if Settings.ESP.HitLogs.Enabled and method == "FireServer" and obj.Name == "ProjectileInflict" and obj:IsA("RemoteEvent") then
        local args = {...}
        AddHitlog(tostring(args[2] or "Unknown"), tostring(args[1] or "Target"))
    end

    return oldNamecall(obj, ...)
end)

-- Detección Universal basada en cambios de salud
local function SetupUniversalHitlogs(player)
    ...
    local function monitorCharacter(char)
        ...
        local conn = hum.HealthChanged:Connect(function(newHealth) 
            if Settings.ESP.HitLogs.Universal and newHealth < lastHealth and hum and hum.Parent then
                local root = char:FindFirstChild("HumanoidRootPart")
                if root then
                    local _, onScreen = Camera:WorldToViewportPoint(root.Position)
                    -- Prioridad de visibilidad para evitar lag por cálculos en jugadores lejanos
                    if onScreen then
                        AddHitlog("Health Update", player.DisplayName)
                    end
                end
            end
...
for _, p in pairs(Players:GetPlayers()) do SetupUniversalHitlogs(p) end
table.insert(GlobalConnections, Players.PlayerAdded:Connect(SetupUniversalHitlogs))
...
local function CreateESP(player)
    if player == Players.LocalPlayer or not has_drawing_lib() then return end
    local esp = {
        Box = {
            L1 = Drawing.new("Line"), L2 = Drawing.new("Line"), L3 = Drawing.new("Line"), L4 = Drawing.new("Line"),
            L5 = Drawing.new("Line"), L6 = Drawing.new("Line"), L7 = Drawing.new("Line"), L8 = Drawing.new("Line"),
            L9 = Drawing.new("Line"), L10 = Drawing.new("Line"), L11 = Drawing.new("Line"), L12 = Drawing.new("Line")
        },
...
local function RemoveESP(player)
    if Drawings.ESP[player] then
        ...
        esp.Info.Name:Remove(); esp.Info.Distance:Remove()
...
local function UpdateESP(player)
    ...
    local pos = Vector2.new(posV3.X, posV3.Y)
    ...
    local topV3 = Camera:WorldToViewportPoint((root.CFrame * CFrame.new(0, size.Y/2, 0)).Position)
    local bottomV3 = Camera:WorldToViewportPoint((root.CFrame * CFrame.new(0, -size.Y/2, 0)).Position)
    local top = Vector2.new(topV3.X, topV3.Y)
    local bottom = Vector2.new(bottomV3.X, bottomV3.Y)
    ...
    if Settings.ESP.Boxes then
        if Settings.ESP.BoxStyle == "ThreeD" then
            ...
            -- Frente
            drawLine(esp.Box.L1, f_tl, f_tr); drawLine(esp.Box.L2, f_tr, f_br); drawLine(esp.Box.L3, f_br, f_bl); drawLine(esp.Box.L4, f_bl, f_tl)
            -- Atrás
            drawLine(esp.Box.L5, b_tl, b_tr); drawLine(esp.Box.L6, b_tr, b_br); drawLine(esp.Box.L7, b_br, b_bl); drawLine(esp.Box.L8, b_bl, b_tl)
            -- Conectores
            drawLine(esp.Box.L9, f_tl, b_tl); drawLine(esp.Box.L10, f_tr, b_tr); drawLine(esp.Box.L11, f_br, b_br); drawLine(esp.Box.L12, f_bl, b_bl)
        elseif Settings.ESP.BoxStyle == "Corner" then
            local cSize = boxWidth * 0.2
            esp.Box.L1.From = boxPos; esp.Box.L1.To = boxPos + Vector2.new(cSize, 0)
            esp.Box.L2.From = boxPos; esp.Box.L2.To = boxPos + Vector2.new(0, cSize)
            
            esp.Box.L3.From = boxPos + Vector2.new(boxWidth, 0); esp.Box.L3.To = boxPos + Vector2.new(boxWidth - cSize, 0)
            esp.Box.L4.From = boxPos + Vector2.new(boxWidth, 0); esp.Box.L4.To = boxPos + Vector2.new(boxWidth, cSize)
            
            esp.Box.L5.From = boxPos + Vector2.new(0, boxHeight); esp.Box.L5.To = boxPos + Vector2.new(cSize, boxHeight)
            esp.Box.L6.From = boxPos + Vector2.new(0, boxHeight); esp.Box.L6.To = boxPos + Vector2.new(0, boxHeight - cSize)
            
            esp.Box.L7.From = boxPos + Vector2.new(boxWidth, boxHeight); esp.Box.L7.To = boxPos + Vector2.new(boxWidth - cSize, boxHeight)
            esp.Box.L8.From = boxPos + Vector2.new(boxWidth, boxHeight); esp.Box.L8.To = boxPos + Vector2.new(boxWidth, boxHeight - cSize)
            
            for _, v in pairs(esp.Box) do v.Visible = true end
        else
            esp.Box.L1.From = boxPos; esp.Box.L1.To = boxPos + Vector2.new(boxWidth, 0)
            esp.Box.L2.From = boxPos + Vector2.new(0, boxHeight); esp.Box.L2.To = boxPos + Vector2.new(boxWidth, boxHeight)
            esp.Box.L3.From = boxPos; esp.Box.L3.To = boxPos + Vector2.new(0, boxHeight)
            esp.Box.L4.From = boxPos + Vector2.new(boxWidth, 0); esp.Box.L4.To = boxPos + Vector2.new(boxWidth, boxHeight)
            esp.Box.L1.Visible = true; esp.Box.L2.Visible = true; esp.Box.L3.Visible = true; esp.Box.L4.Visible = true
        end
    ...
    if Settings.ESP.Health then
        ...
        local barWidth = math.max(2, boxWidth * 0.05)
        esp.HealthBar.Outline.Position = boxPos - Vector2.new(barWidth + 4, 0); esp.HealthBar.Outline.Size = Vector2.new(barWidth + 2, boxHeight)
        esp.HealthBar.Fill.Position = boxPos - Vector2.new(barWidth + 3, -boxHeight); esp.HealthBar.Fill.Size = Vector2.new(barWidth, -boxHeight * hPct)
            ...
            if part then
                ...
                if onScreen then
                    if Settings.Aimbot.Method == "Camera" then
                        ...
                        safe_mouse_move((targetPos.X - mousePos.X) / smoothing, (targetPos.Y - mousePos.Y) / smoothing)
                    end
                end
            end
        end
    ...
    if char and root and hum then
        ...
        if Settings.Misc.AntiCheatDetection and (Settings.Misc.SpeedHack or Settings.Misc.Noclip) then
            ...
            if distance > expectedMax and distance < 50 then
                ...
                if Notify then
                    Notify("SAFETY: Anti-Cheat Detected!", "error")
                    Notify("Movement hacks disabled.", "info")
                end
            end
        ...
        if Settings.Misc.Float or floatState ~= "Idle" then
            ...
            if floatState == "Idle" and Settings.Misc.Float then
                if now - lastFloatTick > Settings.Misc.FloatCooldown then
                    ...
                    if Notify then Notify("Float is on cooldown!", "warning") end
                end
            end
...
local function BuildFatalityUI()
    ...
    table.insert(GlobalConnections, RunService.RenderStepped:Connect(function(dt)
        ...
        if tick() - lastUpdate >= 0.5 then
            lastFPS = dt > 0 and math.floor(1/dt) or 0
            lastUpdate = tick()
        end
    ...
    ColAimbot.AddToggle("Aimbot", function(v) 
        ...
        if v and Notify then Notify("Rage Aimbot active!", "warning") end
    ...
    local HitCol = PlayersSub.AddColumn("Hit Logs")
    HitCol.AddToggle("Enable Hitlogs", function(v) Settings.ESP.HitLogs.Enabled = v end, Settings.ESP.HitLogs.Enabled)
    HitCol.AddToggle("Universal Mode", function(v) Settings.ESP.HitLogs.Universal = v end, Settings.ESP.HitLogs.Universal)
    HitCol.AddColorPicker("Log Color", Settings.ESP.HitLogs.Color, function(c) Settings.ESP.HitLogs.Color = c end)
    ...
    MoveCol.AddToggle("Fly (HIGH RISK)", function(v) 
        Settings.Misc.Fly = v 
        if v and Notify then Notify("Fly enabled. Use with caution!", "warning") end
    ...
    MoveCol.AddToggle("Speed Hack (RISKY)", function(v) 
        ...
        if v and Notify then Notify("Speed Hack enabled!", "warning") end
    ...
    MoveCol.AddToggle("Noclip (HIGH RISK)", function(v) 
        ...
        if v and Notify then Notify("Noclip is active!", "error") end
    end, Settings.Misc.Noclip)
    MoveCol.AddToggle("Infinite Jump", function(v) 
        Settings.Misc.InfiniteJump = v 
        if v and Notify then Notify("Infinite Jump active!", "warning") end
    ...
    LuaCol.AddButton("Run Script", function()
        ...
        if func then
            ...
            if success then
                Notify("Script executed successfully!", "success")
            else
                Notify("Runtime Error: " .. tostring(fault), "error")
            end
        else
            Notify("Syntax Error: " .. tostring(err), "error")
        end
    end)
    ...
    LuaUtil.AddButton("Run from Clipboard", function()
        if getclipboard then
            ...
            local func, err = loadstring(code)
            if func then pcall(func) else Notify("Clipboard Error: "..tostring(err), "error") end
        else
            Notify("getclipboard not supported", "error")
        end
    end)
    ...
    LuaUtil.AddButton("Run from URL", function()
        ...
        if url ~= "" then
            ...
            if success then
                ...
                if func then pcall(func) else Notify("URL Compile Error", "error") end
            else
                Notify("HTTP Get Failed", "error")
            end
        end
    end)
    ...
    LuaFiles.AddButton("Save to File", function()
        if writefile and fileName.Text ~= "" then
            writefile(fileName.Text, scriptBox.Text) -- La función writefile se espera que esté disponible
            Notify("Saved: "..fileName.Text, "success")
        end
    end)
    ...
    LuaFiles.AddButton("Load from File", function()
        if readfile and isfile and isfile(fileName.Text) then
            ...
            Notify("Loaded: "..fileName.Text, "success")
        else
    ...
    MenuCol.AddToggle("Menu Visibility", function(v)
        main.Visible = v
        if CrossGui then CrossGui.Enabled = v end
    end, main.Visible, Settings.UI_Key)

    if isfile and isfile("fatality_v3.json") then
        Notify("Configuration restored from file", "success")
    end
