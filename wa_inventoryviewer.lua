...
function InventoryViewer:Init()
    ...
    local MainFrame = Create("Frame", {
        ...
        Position = UDim2.new(0.5, 200, 0.5, -110), -- Positioned to the right of the main menu
        BackgroundColor3 = Color3.fromRGB(13, 13, 13), -- Matching provided snippet
        BorderSizePixel = 0
    })
    self.MainFrame = MainFrame

    Create("UICorner", {Parent = MainFrame, CornerRadius = UDim.new(0, 6)}) -- Added corner for consistency
    ...
    local TitleBar = Create("Frame", {
        ...
        BackgroundColor3 = Color3.fromRGB(18, 18, 18), -- Matching provided snippet
        BorderSizePixel = 0
    })
    ...
    local TitleLabel = Create("TextLabel", {
        ...
        BackgroundTransparency = 1,
        TextColor3 = Color3.fromRGB(225, 227, 229),
        TextSize = 12,
    ...
    local MainLiner = Create("Frame", {
        ...
        BackgroundColor3 = Color3.fromRGB(131, 194, 242), -- Matching provided snippet
        BorderSizePixel = 0
    })
    ...
    local InvScroll = Create("ScrollingFrame", {
        Parent = MainFrame,
        Size = UDim2.new(1, -10, 1, -95),
        Position = UDim2.new(0, 5, 0, 90),
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        ScrollBarThickness = 2,
        AutomaticCanvasSize = Enum.AutomaticSize.Y,
        ScrollBarImageColor3 = Color3.fromRGB(131, 194, 242) -- Matching provided snippet
    })
    ...
    task.spawn(function()
        while task.wait(0.1) do -- Removed 'and getgenv().Library' as this is a standalone component
...
function InventoryViewer:CreateInvItem(parent, name, iconId)
    ...
    local s = Create("UIStroke", {Parent = f, Color = Color3.fromRGB(34, 34, 34), Thickness = 1})
    ...
    f.MouseEnter:Connect(function()
        tweenService:Create(s, TweenInfo.new(0.2), {Color = Color3.fromRGB(131, 194, 242)}):Play()
        tweenService:Create(f, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(20, 20, 20)}):Play()
    ...
    f.MouseLeave:Connect(function()
        tweenService:Create(s, TweenInfo.new(0.2), {Color = Color3.fromRGB(34, 34, 34)}):Play()
        tweenService:Create(f, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(16, 16, 16)}):Play()
        if self.Tooltip then
...
function InventoryViewer:Init(invListRef)
    self:CacheGameItems()
    self.InvListRef = invListRef -- Set the reference to the Listbox from wa.lua
...
function InventoryViewer:ScanPlayerInventory(player)
    ...
    elseif player:IsA("Model") then -- For NPCs or other models with inventory
        scan(player)
        -- If it's an NPC, it might not have a player folder in ReplicatedStorage
        -- The current logic for player inventory is specific to players.
        -- For models, we just scan their children.
    end
...
return InventoryViewer-- Inventory Viewer for Amber UI (wa.lua)
local InventoryViewer = {
    Enabled = false,
    Target = nil,
    Connections = {},
